...
class HttpUtil {
  ...
  /// 保存网络图片到相册
  static Future saveNetImageToPhotosAlbum(String url, String name) async {
    var op = Options(responseType: ResponseType.bytes);
    var response = await Dio().get(url, options: op);
    var bytes = Uint8List.fromList(response.data);
    return ImageGallerySaverPlus.saveImage(bytes, quality: 100, name: name);
  }

  /// 保存网络媒体到相册
  static Future saveNetMediaToPhotosAlbum(String url, String name) async {
    ...
    return ImageGallerySaverPlus.saveFile(tempFile.path, name: name);
  }
